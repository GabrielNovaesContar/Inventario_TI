...
function aplicarFiltroPecas() {
    ...
    filtrados.forEach(p => {
        ...
        tbody.innerHTML += `<tr class="hover:bg-slate-50 border-b">
            <td class="p-4 font-bold text-slate-800">${p.name}</td><td class="p-4 text-sm text-slate-600">${p.brand}</td>
            <td class="p-4 text-center">
                <button onclick="alterarEstoquePeca('${pJSON}', -1)" class="bg-red-100 text-red-700 px-3 py-1 rounded hover:bg-red-200 transition">-</button>
                <span class="mx-3 font-mono font-bold text-lg">${p.quantity}</span>
                <button onclick="alterarEstoquePeca('${pJSON}', 1)" class="bg-green-100 text-green-700 px-3 py-1 rounded hover:bg-green-200 transition">+</button>
            </td>
            <td class="p-4 text-right">
                ...
                <button onclick="pedirConfirmacao('Deseja excluir esta peça?', () => deletarPeca('${p.id}'))" class="text-red-600 font-semibold text-sm hover:text-red-800">Excluir</button>
            </td>
        </tr>`;
    });
}
...
async function salvarPeca() {
    ...
    if(!b.name) return mostrarToast("Preencha o nome!", "error");
    await fetch(id ? `${API_URL}/parts/${id}` : `${API_URL}/parts`, { method: id ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(b) });
...
async function deletarPeca(id) {
    ...
    mostrarToast("Peça excluída.", "success"); carregarPecas();
}
