...
function desenharTabelaAtivos(ativos) {
    ...
    ativos.forEach(ativo => {
        ...
        tr.innerHTML = `
            <td class="p-4"><div class="font-mono text-sm text-slate-800 font-bold">${ativo.patrimony}</div><div class="text-xs text-slate-500 mt-1">S/N: ${ativo.serial_number || 'N/A'}</div></td>
            <td class="p-4"><div class="font-bold text-slate-800">${ativo.name}</div><div class="text-xs text-slate-500 truncate w-48" title="${ativo.specs || ''}">${ativo.specs || '-'}</div></td>
            <td class="p-4 text-sm text-slate-600">${ativo.category.replace("_", " ")}</td>
            <td class="p-4"><div class="font-semibold text-slate-700">${ativo.sector.replace("_", " ")}</div><div class="text-xs text-slate-500 mt-1">${ativo.responsible || 'Sem Resp.'}</div></td>
            ...
            <td class="p-4 text-right">
                ...
                <button onclick="pedirConfirmacao('Deseja excluir este equipamento?', () => deletarAtivo('${ativo.id}'))" class="text-red-600 hover:text-red-800 font-semibold text-sm">Excluir</button>
            </td>
        `;
...
async function salvarAtivo() {
    ...
    const ativo = {
        name: document.getElementById("ativo-nome").value, patrimony: document.getElementById("ativo-patrimonio").value,
        serial_number: document.getElementById("ativo-sn").value, status: document.getElementById("ativo-status").value,
        category: document.getElementById("ativo-categoria").value, sector: document.getElementById("ativo-setor").value,
        specs: document.getElementById("ativo-specs").value, responsible: document.getElementById("ativo-responsavel").value
    };
    if(!ativo.name || !ativo.patrimony) return mostrarToast("Preencha Nome e Patrimônio!", "error");

    await fetch(id ? `${API_URL}/assets/${id}` : `${API_URL}/assets`, {
        method: id ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(ativo)
    });
    mostrarToast("Equipamento salvo com sucesso!");
    fecharModal(); carregarAtivos(); 
}
...
function abrirModalNovo() {
    ...
    toggleSpecs(); document.getElementById("modal-ativo").classList.remove("hidden");
}

function abrirModalEdicao(ativoJSON) {
    const ativo = JSON.parse(decodeURIComponent(ativoJSON));
...
async function carregarHistorico(assetId) {
    ...
    hist.forEach(reg => {
        ...
        tr.innerHTML = `<td class="p-2 font-mono text-slate-500">${new Date(reg.date).toLocaleDateString('pt-BR',{timeZone:'UTC'})}</td>
                        <td class="p-2 font-semibold">${reg.action}</td><td class="p-2 text-slate-600">${reg.user}</td>
                        <td class="p-2 text-right"><button onclick="pedirConfirmacao('Excluir esta linha?', () => deletarHistorico('${reg.id}', '${assetId}'))" class="text-red-500 hover:text-red-700 font-bold">X</button></td>`;
        tbody.appendChild(tr);
    });
}
...
async function salvarHistorico() {
    ...
    if(!body.date || !body.user || !body.action) return mostrarToast("Preencha todos os campos do histórico!", "error");
    ...
    document.getElementById("hist-acao").value = ""; carregarHistorico(assetId);
}
